...
sub initialize {
    ...
    $self->{scrubber} = do {
        ...
        my $default = delete $config->{default_deny} ? 0 : 1;
        ...
        $scrubber->default($default, { %default, %{ delete $config->{default} || {} } });
...
sub update {
    ...
    if (defined $args->{entry}->body && $args->{entry}->body->is_html) {
        ...
        if ($args->{entry}->summary && $args->{entry}->summary->type eq 'html') {
            my $scrubbed = $self->{scrubber}->scrub( $args->{entry}->summary->data );
